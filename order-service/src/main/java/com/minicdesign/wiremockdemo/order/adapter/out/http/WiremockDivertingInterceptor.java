...
public class WiremockDivertingInterceptor implements ClientHttpRequestInterceptor {
	...
	public WiremockDivertingInterceptor(RestClient wiremockRestClient,
	...
	public ClientHttpResponse intercept(HttpRequest request, byte[] body, ClientHttpRequestExecution execution)
		...
		if (featureEnabled && uri.getPath().contains("PROD-800")) {
			...
			try {
				ResponseEntity<byte[]> responseEntity = wiremockRestClient.method(request.getMethod())
						.uri(pathWithQuery).headers(headers -> headers.putAll(request.getHeaders())).body(body)
						.retrieve().toEntity(byte[].class);

				log.info("[WireMock Interceptor] SUCCESS: Received response from WireMock: {}",
