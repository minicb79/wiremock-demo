...
public class WiremockProxyInitializer implements CommandLineRunner {
    ...
    public WiremockProxyInitializer(
    ...
    public void run(@Nullable String... args) {
        ...
        Map<String, String> requestCriteria = Map.of(
                "method", "ANY",
        ...
        try {
            ...
            Map<String, Object> mappingBody = Map.of(
                    "id", mappingId,
                    "request", requestCriteria,
                    "response", Map.of("proxyBaseUrl", targetBaseUrl),
                    "priority", 10
            );

            var response = wiremockRestClient
                    .post()
            ...
            if (response.getStatusCode().is2xxSuccessful()) {
                log.info("[WiremockProxyInitializer] SUCCESS: Registered catch-all proxy mapping to: {}", targetBaseUrl);
            } else {
                log.warn("[WiremockProxyInitializer] WARNING: Failed to register catch-all proxy. WireMock status: {}", response.getStatusCode());
            }
        } catch (Exception ex) {
            log.error("[WiremockProxyInitializer] ERROR: Failed to register catch-all proxy mapping. Ensure WireMock is running. Error details: {}", ex.getMessage(), ex);
        }
    }
}
