...
public class InventoryHttpAdapter implements InventoryPort {
    ...
    public InventoryHttpAdapter(
        ...
        if (asProxyEnabled) {
            if (useForwardProxy) {
                ...
                int port = proxyUri.getPort() != -1 ? proxyUri.getPort() : ( "https".equals(proxyUri.getScheme()) ? 443 : 80 );

                log.info("[InventoryHttpAdapter] Wiremock as Forward Proxy is ENABLED. Routing traffic through HTTP Proxy at {}:{}", host, port);
                ...
                log.info("[InventoryHttpAdapter] Wiremock as Reverse Proxy is ENABLED. Routing traffic to WireMock: {}", wiremockBaseUrl);
    ...
    public Integer getAvailableQuantity(String productId) {
        ...
        try {
            ...
            log.info("Inventory response for productId={} quantity={} available={}", productId, quantity, response.getAvailable());
